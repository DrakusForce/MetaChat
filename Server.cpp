...
int find(char Rec[]);
bool check_flag(int rev_id,int child_id);
void change_host(int id);
int main()
{
	int server_socket;
...
void change_host(int id)
{
	...
	if(clients[i].mark)
	{
		...
		{
			if(clients[j].id==clients[i].vid[0])
			{
				clients[j].mark=true;
				break;
			}
		}
		for (int k = 1; k < clients[i].vid.size(); k++)
		{
			clients[j].vid.push_back(clients[i].vid[k]);
		}		
	}
}